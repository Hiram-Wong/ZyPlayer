      ...
      <div class="doh-dialog-container dialog-container-padding">
        <div class="data-item top">
          <t-form ref="form" :data="formData" @submit="onSubmit">
            ...
            <t-radio-group v-model="dnsSelect" variant="default-filled" size="small" class="mg-t" @change="changeDnsSelect">
...
watch(
  ...
  (val) => {
    ...
    const index = findIndex(DNS_LIST, ['dns', val.data]);
    if (index > -1) dnsSelect.value = val.data;
  },
);

const changeDnstextarea = (item) => {
  const index = findIndex(DNS_LIST, ['dns', item]);

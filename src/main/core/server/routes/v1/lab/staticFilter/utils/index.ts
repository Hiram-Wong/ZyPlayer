import { pdfh as pdfhModule, pdfa as pdfaModule } from '@main/utils/hiker/htmlParser';
...
const getFilters = (
  ...
  list.forEach((item: any, id) => {
    ...
    if (item.constructor == Array) {
      c = item.map((e) => pdfaModule(html, e)).flat(1);
    } else if (item.constructor == String) {
      c = pdfaModule(html, item as string);
    }
    ...
    c.forEach((it, _i) => {
      ...
      tobj = ty.map((y) => {
        ...
        var obj: any = {
          title: pdfhModule(y, title),
          url: pdfhModule(y, url),
        };
        let me = ms[fs];
...
const processCategories = (contentHtml, class_parse, cate_exclude, reurl, url) => {
  ...
  const categories =
    pdfaModule(contentHtml, listSelector)
      .map((item) => {
        const title = pdfhModule(item, titleSelector).trim();
        if ((cate_exclude && new RegExp(cate_exclude).test(title)) || excludePattern.test(title)) {
          console.log(`${title}: 跳过`);
          return;
        }

        const itemUrl = pdfhModule(item, urlSelector);

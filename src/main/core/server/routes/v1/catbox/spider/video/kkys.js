...
async function play(inReq, _outResp) {
  ...
  try {
    if (
      id.indexOf('youku') >= 0 ||
      id.indexOf('iqiyi') >= 0 ||
      id.indexOf('v.qq.com') >= 0 ||
      id.indexOf('pptv') >= 0 ||
      id.indexOf('le.com') >= 0 ||
      id.indexOf('1905.com') >= 0 ||
      id.indexOf('mgtv') >= 0
    ) {
      if (parse.length > 0) {
    ...
    if (id.indexOf('jqq-') >= 0) {
      ...
      var ids = id.split('-');
      var jxJqq = await req.get(
        'https://api.juquanquanapp.com/app/drama/detail?dramaId=' + ids[1] + '&episodeSid=' + ids[2] + '&quality=LD',
        { headers: jqqHeaders },
      );

...
async function init(inReq, _outResp) {
  ...
  if (!device.id) {
    ...
    device.ua =
      'Dalvik/2.1.0 (Linux; U; Android ' + device.release + '; ' + device.model + ' Build/' + device.buildId + ')';
  ...
  for (const key in data) {
    ...
    try {
      ...
      if (filterAll.length === 0) {
        let filterData = (await request(url + '/api.php/provide/searchFilter?type_id=0&pagenum=1&pagesize=24')).data
          .conditions;
        // 年份
        let year = {
          key: 'year',

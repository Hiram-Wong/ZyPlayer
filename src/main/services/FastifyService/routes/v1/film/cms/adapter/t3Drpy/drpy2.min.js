import {
  ...
  TextDecoder,
  WXXH,
} from './drpy-core-lite.min';
import { batchFetch, joinUrl, local, pd, pdfa, pdfh, req } from './inject';
...
if (!Array.prototype.includes) {
  Object.defineProperty(Array.prototype, 'includes', {
    value(searchElement, fromIndex) {
      ...
      let o = new Object(this); // 将this转变成对象
      let len = o.length >>> 0; // 无符号右移0位，获取对象length属性，如果未定义就会变成0
      ...
      let n = fromIndex | 0; // 查找起始索引
      let k = Math.max(n >= 0 ? n : len - Math.abs(n), 0); // 计算正确起始索引，因为有可能是负值
...
if (typeof String.prototype.endsWith !== 'function') {
  String.prototype.endsWith = function (suffix) {
    return this.includes(suffix, this.length - suffix.length);
  };
}
...
function rc(js) {
  if (js === 'maomi_aes.js') {
    let a = CryptoJS.enc.Utf8.parse('625222f9149e961d');
    let t = CryptoJS.enc.Utf8.parse('5efdtf6060e2o330');
    return {
      De(word) {
      ...
      En(word) {
        // print(a);
        // print(word);
        let Encrypted = CryptoJS.AES.encrypt(word, a, {
          iv: t,
          mode: CryptoJS.mode.CBC,
          padding: CryptoJS.pad.Pkcs7,
        });
...
function maoss(jxurl, ref, key) {
  ...
  eval(getCryptoJS());
  try {
    let getVideoInfo = function (text) {
      return CryptoJS.AES.decrypt(text, key, { iv, padding: CryptoJS.pad.Pkcs7 }).toString(CryptoJS.enc.Utf8);
    };
    let token_key = key == undefined ? 'dvyYRQlnPRCMdQSe' : key;
    if (ref) {
    ...
    let token_iv = html.split('_token = "')[1].split('"')[0];
...
function readFile(filePath) {
  ...
  let fd = os.open(filePath);
  let buffer = new ArrayBuffer(1024);
  let len = os.read(fd, buffer, 0, 1024);
  console.log(len);
  ...
  classification(img) {
    ...
    try {
      ...
      if (this.api.endsWith('drpy/text')) {
        html = request(this.api, { data: { img }, headers: { 'User-Agent': PC_UA }, method: 'POST' }, true);
      } else {
...
function categoryParse(cateObj) {
  ...
  if (rule.filter_url) {
    ...
    let new_url;
    new_url = cheerio.jinja2(url, { fl, fyclass: cateObj.tid });
  ...
  if (rule.pagecount && typeof rule.pagecount === 'object' && rule.pagecount.hasOwnProperty(MY_CATE)) {
    ...
    pagecount = Number.parseInt(rule.pagecount[MY_CATE]);
  }
  let nodata = {
    list: [
      {
  ...
  let vod =
    d.length < 1
      ...
      : JSON.stringify({
          page: Number.parseInt(cateObj.pg),
...
function searchParse(searchObj) {
  ...
  // print(d);
  return JSON.stringify({
    page: Number.parseInt(searchObj.pg),
    pagecount: 10,
    limit: 20,
    total: 100,
    list: d,
...
function playParse(playObj) {
  ...
  let input = MY_URL; // 注入给免嗅js
  let flag = MY_FLAG; // 注入播放线路名称给免嗅js
  let common_play = {
 ...
 * @param {string} [options.mode] 移除模式:
...
function init(ext) {
  ...
  try {
    ...
    if (typeof ext == 'object') {
      ...
      if (ext.startsWith('http') || is_file) {
        ...
        if (query.type === 'url' && query.params) {
          ...
          if (is_file && query.params.startsWith('http')) {
    ...
    rule.homeUrl = cheerio.jinja2(rule.homeUrl, { rule });
